...
import {
  ...
  FileText, ChevronUp, ChevronDown, Shield, Eye, Send
} from "lucide-react";
...
export default function App() {
  ...
  // Start camera when inspecting phase begins and video element is mounted
  useEffect(() => {
    if (phase === "inspecting" && !cameraReady) {
      const initCamera = async () => {
        try {
          ...
          setError("Camera access denied. You can still type messages.");
        }
      };
      initCamera();
    }
  ...
  const connectWS = useCallback((sid) => {
    const wsUrl = `${API_BASE}/ws/inspect/${sid}`;
    ...
    ws.onmessage = (e) => {
      ...
      switch (msg.type) {
        ...
        case "transcript":
          ...
          break;
        case "finding":
          console.log("Finding received:", msg.finding);
  ...
  const sendText = useCallback(() => {
    ...
    setMessages(prev => [...prev, { text, speaker: "user" }]);
    setTextInput("");
  }, [textInput]);
  ...
  const sendFrameWithText = useCallback(() => {
    ...
    if (!video || video.readyState < 2 || !wsRef.current) {
      ...
      {phase === "welcome" && (
        <div className="flex flex-col items-center justify-center h-full px-6" style={{ animation: "fadeIn 0.5s" }}>
          ...
          <p className="text-white/60 text-sm text-center max-w-sm mb-10 leading-relaxed">
            Point your camera at property damage and chat with your AI inspector.
            Damage is logged automatically and a professional report is generated.
          </p>
          <button onClick={startInspection} className="w-full max-w-sm py-4 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-lg flex items-center justify-center gap-2">
      ...
      {phase === "inspecting" && (
        <div className="flex h-full">
          ...
          <div className="w-2/5 relative bg-gray-900">
            ...
            {/* LIVE badge */}
            <div className="absolute top-3 left-3 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/70">
          ...
          <div className="w-3/5 flex flex-col bg-gray-950">
            ...
            {showFindings && findings.length > 0 && (
              <div className="max-h-48 overflow-y-auto bg-gray-900/80 border-b border-gray-800 p-3 space-y-2">
                {findings.map((f, i) => {
                  const sev = f.severity || f.finding?.severity || "minor";
                  const c = SEV[sev] || SEV.minor;
                  const room = f.room || f.finding?.room || "Unknown";
                  const dtype = f.damage_type || f.finding?.damage_type || "Unknown";
                  const desc = f.description || f.finding?.description || "";
                  const num = f.evidence_number || f.finding?.evidence_number || i + 1;
                  return (
                    <div key={i} className={`p-2.5 rounded-lg ${c.bg} border ${c.border}`}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[11px] font-bold text-white/90">#{num} — {dtype.replace(/_/g, " ")}</span>
                        <span className={`text-[10px] font-bold uppercase ${c.text}`}>{sev}</span>
                      </div>
                      <p className="text-[11px] text-white/60">{room.replace(/_/g, " ")} — {desc.substring(0, 100)}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {messages.length === 0 && (
                <div className="text-center text-white/30 mt-20">
                  <p className="text-sm">Describe damage or point your camera and click "Capture & Ask"</p>
                </div>
              )}
              {messages.map((m, i) => (
            ...
            <div className="p-3 bg-gray-900 border-t border-gray-800">
              <div className="flex gap-2 mb-2">
                <input
                  ...
                  placeholder="Describe damage or ask a question..."
